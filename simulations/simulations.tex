...
\begin{figure}
\centering
\input{Plots/ev_dist_min2}
\hspace{0.2cm}
\input{Plots/ev_dist_max2}
...
%
%
We usually omit the points close to $T/2$ from the fit since they are afflicted with large errors. To obtain the continuum value, we extrapolate $m_{x}^{\rm eff}(g,T)$ for different lattice sizes (see \autoref{fig: massfit_cosh}) to find
%
%
\begin{equation}
...
\begin{equation}
\begin{alignedat}{2}
\langle \tilde{x}_{k}(t)\, \tilde{x}_{i}(0)\rangle_{\rm c} &= 0 \quad \text{for}\quad k\neq i, \; k,i\in \lbrace \rm R,I\rbrace, \\
