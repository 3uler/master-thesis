...
\label{wedge_zero}
\end{equation}
Thereby follows the demanded anti-commutativity for the fermionic \names{Grassmann} variables
\begin{equation}
...
Now we want to see how we can differentiate and integrate \names{Grassmann} variables. We define the derivative to obey
\begin{equation}
...
&\quad + (-1)^{r-1} \delta_{m_{r} n}\,\xi_{m_{1}}\cdots \xi_{m_{r-1}}.
\end{align}
The tangent vectors are also elements of the exterior algebra and satisfy the same anti-commutation rules
\begin{equation}
...
\end{equation}
since the integral is independent of the integration variable. With (\ref{wedge_zero}) it also follows that
\begin{equation}
...
\end{equation}
We now want to perform an integration over the complex \names{Grassmann} variables $\eta$ and $\eta^{\dagger}$. Therefore we investigate an integration over the function
\begin{equation}
e ^{-a\eta\eta^{\dagger}}.
\end{equation}
With
...
\end{equation}
We can see that this introduces an imaginary term in the exponent and therefore a highly oscillating function in the integral. Analytically this term would not bother us, but since we want to perform a numerical calculation something like this often results into a numerical sign problem, discussed in section \ref{sec: sign_prob}.
